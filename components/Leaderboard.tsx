    ...
    <div className="p-4 space-y-6 pb-20">
      ...
      <div className="flex justify-center mb-4">
        <button 
          ...
          disabled={isLoading}
          className="bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-black uppercase px-4 py-2 rounded-full border border-slate-700 transition-all flex items-center gap-2"
        >
          {isLoading ? (
          ...
          <div className="space-y-2">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-slate-800 text-xl relative shadow-inner">
                    ...
                    {chef.rank === 2 && <span className="absolute -top-2 -left-2">🥈</span>}
                    {chef.rank === 3 && <span className="absolute -top-2 -left-2">🥉</span>}
                  </div>
                  <div className="overflow-hidden">
                    <h3 className={`font-bold text-sm leading-none truncate ${chef.user_id === userId ? 'text-emerald-400' : 'text-white'}`}>
                      #{chef.rank} {chef.username} {chef.user_id === userId && "(You)"}
                    </h3>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter mt-1 truncate">{chef.title}</p>
                  </div>
                </div>
            ...
            {userRank === null && !isLoading && (
              <div className="mt-4 p-4 bg-slate-900/80 rounded-2xl border border-slate-800 text-center">
                ...
                <p className="text-[10px] text-slate-600 mt-1 uppercase">Earn more than ${leaders.length > 0 ? leaders[leaders.length-1].total_earned.toLocaleString() : '0'} to enter the Top 20!</p>
              </div>
            )}
          </div>
        )}
      ...
      <div className="p-4 bg-slate-900/50 rounded-2xl border border-slate-800 text-center">
        {lastUpdated && (
          <p className="text-[8px] text-slate-600 uppercase font-black mb-2 tracking-widest">
